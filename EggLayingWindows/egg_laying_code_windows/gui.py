...
class VideoPlayer:
    def __init__(self, working_path, checked_path, working_assay):
        ...
        folder_path = os.path.normpath(os.path.join(checked_path, working_assay))
        ...
        for idx, item in enumerate(self.working_assays):
            ...
            assay_path = os.path.normpath(os.path.join(path_out, item))
        ...
        originals_path = os.path.normpath(os.path.join(self.working_path, self.working_assay))
    ...
    def read_processed_videos_from_folder(self, originals_path):
        ...
        if os.path.exists(originals_path):
            for filename in os.listdir(originals_path):
                filepath = os.path.normpath(os.path.join(originals_path, filename))
    ...
    def set_originals_path(self, working_path, working_assay, init):
        ...
        if os.path.exists(working_path):
            ...
            checked_path = os.path.normpath(os.path.normpath(os.path.join(padre, "egg_laying_new")))
            ruta = os.path.join(checked_path)
        ...
        originals_path = os.path.normpath(os.path.join(working_path, working_assay))
    ...
    def save_assay_result(self):
        ...
        output_filename = os.path.normpath(os.path.join(path_out, 'metadata_eggs_final.csv'))
        ...
        with open(output_filename, 'w', newline='') as csvfile:
            ...
            for idx, item in enumerate(self.originals_videos):
                ...
                if lib.is_video_saved(path_out, name_video):
                    file_name_frames = os.path.normpath(os.path.join(path_out, name_video + '_metadata_eggs_frames_final.csv'))
    ...
    def open_video(self, init, frame_number):
        ...
        if not init:
            ...
            if self.show_worm_details.get() == 1:
                ...
                self.cap_original = cv2.VideoCapture(os.path.normpath(os.path.join(self.working_path, self.working_assay, self.video)))
                if not self.cap_original.isOpened():
                    print("Error can't open video ", os.path.normpath(os.path.join(self.working_path, self.working_assay, self.video)))

        self.cap = cv2.VideoCapture(os.path.normpath(os.path.join(self.videos_path, self.video)))
        self.video_number = 0
    ...
    def process_assays(self):
        for working_assay in self.working_assays:
            originals_videos = self.read_videos_from_folder(os.path.normpath(os.path.join(self.working_path, working_assay)))
    ...
    def process_videos(self):
        ...
        for originals_video in self.originals_videos:
            ...
            if not lib.is_process_init(originals_path, name_video):
                ...
                path_name_video = os.path.normpath(os.path.join(originals_path, name_video))
    ...
    def delete_item_NOK(self):
        ...
        if selected_index:
            ...
            path_name_video = os.path.normpath(os.path.join(self.videos_path, self.video.split('.')[0]))
    ...
    if os.path.exists("rutas.txt"):
        ...
        with open("rutas.txt", "r") as f:
            ...
            if len(rutas) >= 1:
                ...
                padre = os.path.dirname(working_path)
                checked_path = os.path.normpath(os.path.join(padre, "egg_laying_new"))
                ruta = os.path.join(checked_path)
                os.makedirs(ruta, exist_ok=True)
