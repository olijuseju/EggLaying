...
def get_segmentation(img, pts_diff, intentos, worm_detected):
    ...
    sure_fg = cv2.inRange(gray, 0, 140) # Rango de valores de gusano seguros # 110 JOSE
    ...
    for contour in contours:
        ...
        if (area > 650.0) and (area < 1300.0): # 600, 3000
...
def borra_gusanos(path, name_video, img):
    ...
    #if cv2.countNonZero(sure_fg) < 100:  # Ajustable
...
def eliminate_edges_of_image(image):
    ######################## CODIGO JOSE #######################
    global centro_real,radio_real
    h,w = image.shape[:2]
    ...
    if centro_real[0]==0 and centro_real[1]==0:
       centro_real,radio_real = detectar_circulo_mayor(image)
    print("centro_real: " + str(centro_real))
    print("radio_real: " + str(radio_real))
    if centro_real[0]>0 and centro_real[1]>0 and radio_real>0:
       centro = centro_real
       radio = radio_real
    
    
    cv2.circle(mascara,centro,radio,0,-1)
    image[mascara==255]=255
    return image
    ######################## CODIGO JOSE #######################
...
def get_changes_red(path, name_video, cap, ini_frame, end_frame):
    ...
    #    count+=1
    #    #print(count)
    #    ret, img_endaux = cap.read()
    #    if img_endaux is None:
    #       break
    #    else:
    #        img_end = img_endaux
    ######################## CODIGO JOSE #######################
    img_end = eliminate_edges_of_image(img_end)
    path_name_video = os.path.join(path, name_video)
    cv2.imwrite(path_name_video + "_img_result.bmp", img_end)
    ...
    cv2.imwrite(path_name_video + "_gray_end.bmp", gray_end)
    cap.set(cv2.CAP_PROP_POS_FRAMES, ini_frame)
    ret, img = cap.read()
    if ret == False:
        img = img_end
    img = eliminate_edges_of_image(img)
    # cv2.imshow("img", img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
...
def detectar_circulo_mayor(img, umbral_color=170):
    # Carga la imagen
    ...
    # Umbral para obtener solo regiones con intensidad > 170
    _, binaria = cv2.threshold(gris, umbral_color, 255, cv2.THRESH_BINARY)
    ...
    for cnt in contornos:
        ...
        # Solo considerar si el contorno está cerca de un círculo relleno (verificamos proporción de áreas)
        if area_contorno / area_circulo > 0.8 and radio > max_radio:
            max_radio = radio
            mejor_centro = (int(x), int(y))

    if mejor_centro != None and max_radio > 1650:
        return mejor_centro, int(max_radio)-10
    else:
        return 0, 0
    ...
    while (n_frame <= end_frame) and (cap.isOpened()):
        ...
        if len(pts_diff[0]) > 0:
            ...
            #while (not suficient_skel_lenght) and (intentos <= 1):
                ...
                cv2.imwrite(path_name_video + "_imgs/img_res.bmp", img_res)
                cv2.imwrite(path_name_video + "_imgs/img_seg_dilated.bmp", img_seg_dilated)
                cv2.imwrite(path_name_video + "_imgs/img_seg.bmp", img_seg)
                ...
                if (len(boxes) > 0) and (dist_min < 199 or dist_min_ini == 9999999):
                    ...
                    if len(pts_diff_recortada[0]) > 0:
                        ...
                        img_res_recortada[skel > 0] = (0, 255, 0)
                        cv2.imwrite(path_name_video + "_imgs/img_res_recortada_skel.bmp", img_res_recortada)
                                    ...
                                    if elongacion >= 0.3: # 0.39 # 0.44
                                        ...
                                        if dist < 200: # todo: probar a reducir esta distancia, dist inicial 200
                                            ...
                                            if ratio_diff_gray > 0.35:
                                                ...
                                                length_skel_egg = len(np.where(skel_egg>0)[0])
        ...
        if simplify:
            if out.isOpened():
                ...
                print(f"cx_ini: {cx_ini}")
                print(f"cy_ini: {cy_ini}")
                cx, cy = centro_real
                distancia_cuadrada = (cx_ini - cx)**2 + (cy_ini - cy)**2
                if radio_real>1600:
                    radio=radio_real
                else:
                    radio=1750
                ...
                if cy_ini+shape[1]>4024:
                    print(f"worm out of bounds: {cy_ini}")
                    cy_ini=4023-shape[1]
                    worm_detected=False
                if cy_ini<=0:
                    cy_ini=0
                ...
                if cx_ini+shape[0]>3036:
                    print(f"worm out of bounds: {cx_ini}")
                    cx_ini=3035-shape[0]
                    worm_detected=False
                if cx_ini<=0:
                    cx_ini=0
